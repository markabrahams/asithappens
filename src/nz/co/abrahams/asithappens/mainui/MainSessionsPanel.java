...
/**
 * The graphical pane for viewing graphs based on stored information in the
 * database.
 *
 * @author mark
 */
public class MainSessionsPanel extends javax.swing.JPanel implements Runnable {
    ...
    private void exportButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportButtonActionPerformed
        ...
        if (((JTable) (sessionsPane.getViewport().getView())).getSelectedRowCount() > 1) {
            JOptionPane.showMessageDialog(this, "Please select a single session for data export", "Multiple sessions selected", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (returnStatus == JFileChooser.APPROVE_OPTION) {
                ...
                if (!savedFile.exists() || (savedFile.exists() && JOptionPane.showConfirmDialog(null,
                    ...
                    textAccess = new TextAccessObject();
                    textAccess.exportSession(sessionID, savedFile);
                }
            }
        } catch (IOException e) {
    ...
    private void graphButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_graphButtonActionPerformed
        ...
        sessionID = getSessionID();
        if (sessionID == -1) {
            JOptionPane.showMessageDialog(this, "Please select a session for graph display", "No session selected", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            logger.debug("Creating task for data fetch from sessionID " + sessionID);
        ...
        try {
            ...
            while (true) {
                ...
                for (int i = 0; i < sessions.length; i++) {
                    if (isCollecting[i]) {
                        ((JTable) (sessionsPane.getViewport().getView())).setValueAt(DataSets.retrieveDescription(dataSetsDAO, sessions[i]), i, 1);
                    }
                }

            }
        } catch (DBException e) {
            logger.warn("Sessions time updater thread failed");
            //e.printStackTrace();
        }
    }
    ...
    public void displaySessions() {
        ...
        try {
            ...
            for (int i = 0; i < sessions.length; i++) {
                ...
                tableData[i][1] = DataSets.retrieveDescription(dataSetsDAO, sessions[i]);
             ...
             * sessionsList.addListSelectionListener(new
             * javax.swing.event.ListSelectionListener() { public void
     ...
     *
     * @param row table row
     * @param column table column b
     * @param title new title for session
     */
    ...
    /**
     * Returns currently selected sessionID.
     *
    ...
    private int getSessionID() {
        ...
        if (selection == -1) {
            ErrorHandler.modalError(this, "Please select a session", "No session selected");
            return -1;
        }

        sessionID = sessions[selection];
        if (!DAOFactory.getDataSetsDAO().retrieveSessionExists(sessionID)) {
            ErrorHandler.modalError(this, "Please another session", "Can\'t find session in database");
            displaySessions();
            return -1;
        }
    ...
    public static void createGraphWithContext(Component parent, int sessionID, DataSets dataSets, TimeSeriesContext.Aggregation aggregation, TimeSeriesContext.Interpolation interpolation) {
        ...
        try {
            //logger.info("Creating graph for session " + sessionID);
     ...
     */
    /*
     * private void resetDatesButtonActionPerformed(java.awt.event.ActionEvent
     * evt) { int sessionID;
     *
     * sessionID = getSessionID(); if ( sessionID == -1 ) return;
     *
     * setDates(sessionID); }
     */
    /**
    ...
    /*
     * private void
     * refreshSessionsButtonActionPerformed(java.awt.event.ActionEvent evt) {
     * displaySessions(); }
     */
